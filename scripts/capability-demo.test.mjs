...
import { evaluateCapabilities, stableStringify } from './capability-demo.mjs';
...
test('capability demo denies unlisted authority and is deterministic', () => {
  ...
  assert.deepEqual(first.audit.map((event) => event.decision), ['allow', 'deny', 'deny', 'deny']);
  assert.equal(first.audit[0].dispatched, true);
  assert.equal(first.audit.slice(1).every((event) => event.dispatched === false), true);
