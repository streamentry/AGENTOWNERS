...
describe('init command', () => {
  ...
  it('creates file at custom output path', async () => {
    const program = makeProgram();
    await program.parseAsync(['node', 'agentowners', 'init', '--output', 'custom/policy.yml']);
  ...
  it('creates directory recursively', async () => {
    ...
    expect(vi.mocked(fs.mkdirSync)).toHaveBeenCalledWith(expect.stringContaining('.github'), {
      recursive: true,
    });
  });
  ...
  it('errors on unknown profile', async () => {
    const program = makeProgram();
    await program.parseAsync(['node', 'agentowners', 'init', '--profile', 'nonexistent']);
  ...
  it.each(['monorepo', 'dependency-bots'])('creates the %s profile', async (profile) => {
    const program = makeProgram();
    await program.parseAsync(['node', 'agentowners', 'init', '--profile', profile]);

    const writeCall = vi.mocked(fs.writeFileSync).mock.calls[0];
    expect(writeCall?.[1]).toBe(getProfile(profile));
  });
