...
describe('detectAgent', () => {
  it('policy match → confirmed with agentName', () => {
    ...
    expect(result.identityTrust).toBe('verified');
  ...
  it('verified actor mapping outranks an earlier spoofable metadata mapping', () => {
    const policy: AgentOwnersPolicy = {
      version: 1,
      agents: {
        metadata: { match: { commitEmails: ['automation@example.test'] } },
        actor: { match: { actors: ['trusted-bot[bot]'] } },
      },
    };
  ...
  it('known bot actor outranks a spoofable configured body pattern', () => {
    const policy: AgentOwnersPolicy = {
      version: 1,
      agents: {
        metadata: { match: { bodyPatterns: ['generated by the bot'] } },
      },
    };
    ...
    expect(result).toMatchObject({ confidence: 'confirmed', identityTrust: 'verified' });
  ...
  it.each([
    [
      'commit email',
      { commitEmails: ['bot@example.test'] },
      { commitEmails: ['bot@example.test'] },
    ],
    ['commit name', { commitNames: ['Automation Bot'] }, { commitNames: ['Automation Bot'] }],
    ['label', { labels: ['agent-ci'] }, { labels: ['agent-ci'] }],
  ] as const)(
    'configured %s match is confirmed detection but unverified identity',
    (_signal, configured, input) => {
      const policy: AgentOwnersPolicy = {
        version: 1,
  ...
  it('policy body pattern match → confirmed', () => {
    ...
    expect(result.identityTrust).toBe('unverified');
  });
